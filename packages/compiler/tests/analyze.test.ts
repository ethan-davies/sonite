...
function writeTempProject(files: Record<string, string>): string {
  const root = join(tmpdir(), `tsn-analyze-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("analyzeFile + semantic queries", () => {
  it("stamps diagnostics with file paths", () => {
    const root = writeTempProject({
      "main.tsn": `import { add } from "lib";
function main(): void {
  let x: i32 = "bad";
  print(add(1, 2));
}
`,
      "lib.tsn": `export function add(a: i32, b: i32): i32 {
  return a + b;
}
`,
    });

    const result = analyzeFile(join(root, "main.tsn"));
    expect(result.success).toBe(false);
    const typed = result.diagnostics.filter((d) => d.file?.endsWith("main.tsn"));
    expect(typed.length).toBeGreaterThan(0);
    expect(result.diagnostics.every((d) => typeof d.file === "string")).toBe(true);
  });
...
`;
    const root = writeTempProject({ "main.tsn": source });
    const path = join(root, "main.tsn");
    ...
    const completions = completionsAt(result.semantic, path, source.indexOf("print(n)"));
    ...
    expect(symbols.map((s) => s.name).sort()).toEqual(["Point", "add", "main"].sort());
  });

  it("allows analyzing modules without main", () => {
    const root = writeTempProject({
      "lib.tsn": `export function twice(x: i32): i32 {
  return x + x;
}
`,
    });
    const result = analyzeFile(join(root, "lib.tsn"));
    expect(result.diagnostics.some((d) => d.code === "E0200" || d.code === "E0202")).toBe(false);
    expect(result.success).toBe(true);
  });

  it("attributes import resolution errors to the importing file", () => {
    const root = writeTempProject({
      "main.tsn": `import { missing } from "nope";
function main(): void {}
`,
    });
    const result = analyzeFile(join(root, "main.tsn"));
    ...
    expect(imp?.file?.endsWith("main.tsn")).toBe(true);
  });
...
`;
    ...
    expect(completions.items.find((i) => i.name === "contains")?.kind).toBe("method");
    expect(completions.items.find((i) => i.name === "length")?.kind).toBe("property");
  });
...
`;
    ...
    const top = completionsAt(result.semantic, path, source.indexOf("let p"), source);
    ...
    const members = completionsAt(result.semantic, path, source.indexOf("p.") + 2, source);
...
`;
    ...
    const path = join(root, "main.tsn");
    const result = analyzeFile(path);
    const completions = completionsAt(result.semantic, path, source.indexOf("\n}") - 1, source);
    expect(completions.items.some((i) => i.name.startsWith("__prelude_ext_"))).toBe(false);
    expect(completions.items.find((i) => i.name === "function")?.kind).toBe("keyword");
    expect(completions.items.find((i) => i.name === "print")?.kind).toBe("function");
  });
...
`,
    ...
    const completions = completionsAt(result.semantic, path, offset, source);
    expect(completions.items.find((i) => i.name === "add")?.kind).toBe("function");
  });

  it("resolves go-to-definition across namespace imports", () => {
    const root = writeTempProject({
      "main.tsn": `import "lib" as lib;
...
`,
    ...
    const mainPath = join(root, "main.tsn");
    const libPath = join(root, "lib.tsn");
    const result = analyzeFile(mainPath);
    expect(result.success).toBe(true);
    const source = result.semantic.modules.find((m) => m.path === mainPath)!.source;
...
`,
  ...
  it("suggests auto-import completions from the export index", () => {
    const root = writeTempProject({
      "main.tsn": `function main(): void {
  hel
}
`,
      "util.tsn": `export function helper(x: i32): i32 {
  return x + 1;
}
`,
    });
    ...
    const completions = completionsAt(result.semantic, path, offset, source, { exportIndex });
    const helper = completions.items.find((i) => i.name === "helper" && i.autoImport);
